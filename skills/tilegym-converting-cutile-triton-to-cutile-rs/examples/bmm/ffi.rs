...
// ownership gate — no `mem::forget` needed). Logical dims (rt_q/m/n/k) are derived
// from the descriptors' shapes + the transpose flags for the grid math.
//
...
pub unsafe extern "C" fn cutile_bmm(
    ...
    // Reject out-of-contract flag values before any dispatch (0/1 booleans);
    // a garbage flag would otherwise silently mis-select a variant / layout.
    if !matches!(trans_a, 0 | 1) || !matches!(trans_b, 0 | 1) || !matches!(persistent, 0 | 1) {
        return -6;
    }
    // Tile/grid parameters must be positive; num_programs only feeds the persistent grid.
    if bm <= 0 || bn <= 0 || bk <= 0 || group_size_m <= 0 || (persistent != 0 && num_programs <= 0)
    {
        return -6;
    }
    ...
    // All three tensors must share the same dtype.
    if b_d.dtype != a_d.dtype || c_d.dtype != a_d.dtype {
        return -2;
    }
    ...
    // Shape agreement across tensors: b's batch/K dims and c == [Q, M, N].
    //   B physical: trans_b==0 -> [Q,K,N] so K=b_d.dim(1); trans_b==1 -> [Q,N,K] so K=b_d.dim(2).
    let b_k = if trans_b != 0 { b_d.dim(2) } else { b_d.dim(1) };
    if b_d.dim(0) != rt_q
        || b_k != rt_k
        || c_d.dim(0) != rt_q
        || c_d.dim(1) != rt_m
