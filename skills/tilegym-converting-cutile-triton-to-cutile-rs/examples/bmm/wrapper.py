...
Native transpose (Rule 27): transpose_a / transpose_b are passed straight to the
kernel as const flags — the physical tensor crosses unchanged, no ``.contiguous()``
copy to realize a transpose. Tensors cross the cffi boundary as ``const TensorDesc*``
(dtype/shapes/strides travel inside); ``make_tensor_desc`` packs, ``ffi_util::borrow_tensor``
unpacks on the Rust side.
"""
...
# (the shared TensorDesc typedef is prepended by bind_kernel_function_cffi).
_FFI_CDEF = """
int32_t cutile_bmm(
...
def _persistent_configs():
    # analysis.json static_persistent autotune_configs (num_ctas=2 carried to FFI).
    return [
...
def _non_persistent_configs():
    # analysis.json non_persistent: fixed TILE 128/128/32, compile options auto.
    return [
...
def bmm(a, b, transpose_a=False, transpose_b=False, static_persistent=True, use_tma=None, **kwargs):
    ...
    if int(b.shape[0]) != q:
        raise ValueError(f"cutile-rs bmm: batch dim mismatch: a.shape={tuple(a.shape)} b.shape={tuple(b.shape)}")
    k_b = int(b.shape[2]) if transpose_b else int(b.shape[1])
    if k_b != k:
        raise ValueError(f"cutile-rs bmm: K dim mismatch: a.shape={tuple(a.shape)} b.shape={tuple(b.shape)}")
