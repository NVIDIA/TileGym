...
"""OLMoE fused residual add and Llama-style RMSNorm cuTile kernel."""
...
def residual_add_rms_norm_olmoe_cutile(
    ...
    if residual.shape != x.shape:
        raise ValueError(f"residual and x must have identical shapes, got {residual.shape} vs {x.shape}")
    D = residual.shape[-1]
    if weight.numel() != D:
        raise ValueError(f"weight must have length {D}, got {weight.numel()}")
