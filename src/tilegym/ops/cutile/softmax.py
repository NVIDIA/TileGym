...
@ct.kernel
def _softmax_kernel_multi_wave(
    output,
    input,
    N: ConstInt,
    TILE_SIZE: ConstInt,
...
def _launch_softmax_kernel_multi_wave(input, output, TILE_SIZE):
    n_rows, n_cols = input.shape
    input = input.contiguous()
    output = output.contiguous()
...
class _Softmax(torch.autograd.Function):
    ...
    def forward(
        ...
        if use_multi_wave:
            _launch_softmax_kernel_multi_wave(x, y, TILE_SIZE=TILE_SIZE)
        elif use_chunked:
