...
@dispatch("fmha_variant", fallback_backend="triton")
def fmha_variant(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    **kwargs: Any,
    ...
    """
    Fused Multi-Head Attention variant implementation.

    Args:
        q: Query tensor of shape (B, N, S_qo, D) if layout is 'bnsd'
        k: Key tensor of shape (B, N, S_kv, D) if layout is 'bnsd'
        v: Value tensor of shape (B, N, S_kv, D) if layout is 'bnsd'
        scaling: Scale factor for attention scores
        is_causal: Whether to apply causal masking
        q_lens: Query sequence lengths
        kv_lens: Key-value sequence lengths
        bias_type: Type of attention bias, now support vector, matrix, alibi
        bias: Attention bias tensor
        dropout: Dropout probability, random matrix is generated by seed in triton kernel
        seed: Random seed for dropout
        window_size: Sliding window size
        random_mask: Random attention mask of shape (B, N, S_qo, S_kv) in any layout
        layout: Tensor layout format, now support 'bnsd', 'nsbd'
        **kwargs: Additional arguments, including kernel_configs if needed

    Returns:
        Output tensor after attention computation
    """
    raise NotImplementedError(f"fmha_variant is not implemented for {get_current_backend()}")
