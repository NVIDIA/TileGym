...
def _attn_fwd_inner(
    ...
    kv_start = (
        0
        if (WINDOW_SIZE == 0)
        else (tl.maximum(0, prefix_kvlen + start_m * BLOCK_M - WINDOW_SIZE) // BLOCK_N * BLOCK_N)
    )
    kv_end = (
        kv_len_val
        if (WINDOW_SIZE == 0)
    ...
    if STAGE == 1 or STAGE == 2:
        ...
        if STAGE == 1:
            ...
            """
            Theoretically,  hi shoule be kv_end if q_seq_end_cur_block > kv_end else q_seq_end_cur_block
    ...
    for curr_n in range(lo, hi, BLOCK_N, warp_specialize=warp_specialize):
        ...
        k = K_desc.load(_permute_by_layout(k_offset_b, k_offset_h, k_offset_n, k_offset_d, LAYOUT))
        ...
        if BIAS_TYPE == "vector":
            ...
            bias = Bias_desc_or_ptr.load([batch_idx, head_idx, start_m * BLOCK_M, curr_n])
            bias = tl.reshape(bias, (BLOCK_M, BLOCK_N))
            bias = bias.to(tl.float32)
        elif BIAS_TYPE == "alibi":
            alibi_scale = tl.load(Bias_desc_or_ptr).to(tl.float32)
            neg_dist = -tl.abs((curr_n + offs_n)[None, :] - (offs_m[:, None] + prefix_kvlen))
        ...
        if DO_DROPOUT:
            random = tl.rand(
                seed,
                pid_y.to(tl.int64) * S_qo * S_kv + offs_m[:, None] * S_kv + (curr_n + offs_n)[None, :],
            )
        ...
        v = V_desc.load(_permute_by_layout(v_offset_b, v_offset_h, v_offset_n, v_offset_d, LAYOUT))
...
def _create_layout_aware_pre_hook():
    ...
    def layout_aware_pre_hook(nargs):
        ...
        nargs["K_desc"].block_shape = _create_tma_block_by_layout(LAYOUT, BLOCK_N, BLOCK_D)
        nargs["V_desc"].block_shape = _create_tma_block_by_layout(LAYOUT, BLOCK_N, OUT_BLOCK_D)
        nargs["Out"].block_shape = _create_tma_block_by_layout(LAYOUT, BLOCK_M, OUT_BLOCK_D)
...
def _get_configs(kernel_type="prefill"):
    ...
    if get_available_triton_backend() == "nvt":
        return [
            triton.Config(dict(BLOCK_M=BM, BLOCK_N=BN, warp_specialize=False), pre_hook=_hook)
            for BM in [64, 128, 256]
            for BN in [64, 128]
        ]

    # warp_specialize requires SM90+ TMA hardware; force False on Ampere (sm_80)
    ws_choices = [False] if torch.cuda.get_device_capability() == (8, 0) else [True, False]
    # for dev and debug
    if DISABLE_TUNE:
        configs = [
            triton.Config(
...
def _prefill_fmha(
    ...
    if pid_x * BLOCK_M < q_len_val:
        ...
        q = Q.load(_permute_by_layout(q_offset_b, q_offset_h, q_offset_m, q_offset_d, LAYOUT))
...
def _build_tensor_descriptor(tensor):
    ...
    return TensorDescriptor(
        ...
        tensor.shape,
        tensor.stride(),
        [1] * len(tensor.shape),  # dummy block will be overridden by layout-aware pre-hook
    )
...
class _attention(torch.autograd.Function):
    ...
    def forward(
        ...
        if layout not in _VALID_LAYOUTS:
            raise ValueError(f"Invalid layout: {layout}. Valid layouts are: {_VALID_LAYOUTS}")
        assert layout[3] == "d"
...
@register_impl("fmha_variant", backend="triton")
...
def get_fmha_variant_sdpa_paged_interface(backend=None, kernel_configs=None):
    def fmha_interface_wrapper(
        ...
        **kwargs,
        ...
        read_index = kwargs.get("read_index", None)
        write_index = kwargs.get("write_index", None)

        if cache is not None:
            k, v = cache.update(k, v, module.layer_idx, read_index=read_index, write_index=write_index)
