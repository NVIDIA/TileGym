...
- Counter-signal: fp8_quantization_matmul gather-B load converted to a transposed TMA `ct.load(order=(1,0))` on B200 was reverted — TMA-transposed loads must be re-validated per kernel, not adopted as a default. [2026-07]
